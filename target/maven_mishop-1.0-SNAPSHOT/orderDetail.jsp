...
<div class="panel panel-default" style="margin: 0 auto;width: 95%;">
	...
	<table cellpadding="0" cellspacing="0" align="center" width="100%" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<td>订单时间:</td>
			<td>
				<fmt:formatDate value="${od.oTime}" pattern="yyyy-MM-dd HH:mm:ss" var="t"></fmt:formatDate>
				${t}
				</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td colspan="3">
				<table align="center" cellpadding="0" cellspacing="0" width="100%"  class="table table-striped table-bordered table-hover">
					...
					<c:forEach items="${od.itemList}" var="item" varStatus="i">
						<tr align="center">
							<th>${i.count}</th>
							<th>
							...
							<th>
								<fmt:formatDate value="${item.product.pTime}" pattern="yyyy-MM-dd HH:mm:ss" var="t"></fmt:formatDate>
									${t}
							</th>
		...
		<tr>
			<td align="right" colspan="4" style="margin-right: 40px;">
				<a href="${pageContext.request.contextPath }/orderController?method=getOrderList" class="btn btn-danger btn-sm">返回订单列表</a>
				&nbsp;&nbsp;
				<c:if test="${od.oState eq 0 }">
					<button type="button" onclick="pay('${od.oId}','${od.oCount}')" class="btn btn-warning btn-sm">易付宝支付</button>
						&nbsp;&nbsp;
					<button type="button" onclick="payWeiXin('${od.oId}','${od.oCount}')" class="btn btn-success btn-sm">微信支付</button>
				</c:if>
			</td>
		</tr>
	</table>
